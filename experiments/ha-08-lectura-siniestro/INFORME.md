...
`results/consolidado.csv` ya **no** contiene estos datos inválidos — se regeneró con `scripts/collect_results.sh` (corregido para filtrar por patrón `summary_*_r[0-9]*.json`, excluir explícitamente `summary_C_PRIME_*`, los archivos sin sufijo de repetición de esta sesión, y `ttl0_r1`/`ttl300_r1` contaminadas — reemplazadas por `r1b`) y solo tiene las 15 corridas válidas de §1/§2.3bis (no cuenta las 3 de alta carga, que se consolidan aparte). Esta tabla se conserva únicamente en el cuerpo del informe, por trazabilidad del proceso de detección del bug.
...
### 2.1quater Latencia interna media (B4, corregido tras segunda ronda de auditoría externa)

**Esta sección reemplaza un intento anterior que era inválido.** La primera versión intentaba calcular un p95 interno con `histogram_quantile()` sobre `ha08_read_latency_seconds_bucket` — pero el bucket más fino de ese histograma es `0.005` (5ms, ver `api/app/metrics.py`), y las latencias internas reales están casi todas por debajo de ese umbral. `histogram_quantile()` interpola linealmente *dentro* del bucket donde cae el percentil buscado, así que con prácticamente toda la masa de observaciones en el primer bucket (`≤0.005`), el resultado es una extrapolación sin sentido, no una medición — exactamente el mismo problema, ya documentado en §3 para el lag de proyección, que no apliqué a este análisis la primera vez. La tabla anterior (valores como "A: 8.36/7.44/4.97ms, rango de 3.4ms") era ese artefacto, no una medición real de variabilidad de A.

**Corrección:** `ha08_read_latency_seconds_sum` y `_count` son contadores exactos (no dependen de buckets) — su cociente da la **latencia media exacta** de la ventana, aunque no un percentil. Usando `increase(..._sum[780s])` e `increase(..._count[780s])` en el mismo instante final de cada ventana (`increase()` maneja correctamente los reinicios de contador entre corridas, a diferencia de restar dos lecturas puntuales):

| Brazo | Latencia interna media por corrida (ms) | Promedio | n (observaciones) |
|---|---|---:|---:|
| A | 2.500 / 2.453 / 2.075 | 2.34 | 20.267–34.460 |
| B | 1.696 / 1.502 / 1.489 | 1.56 | 20.275–31.358 |
| C | 2.017 / 2.030 / 1.912 | 1.99 | 19.048–22.306 |

Esta serie **es consistente con §2.3bis, no la contradice** (a diferencia de la tabla anterior, donde C−B daba 0.05ms — invisible frente al 0.45ms de costo de escritura ya establecido en §2.3bis): aquí C−B = **0.42ms**, prácticamente igual al 0.45ms medido comparando C-TTL0 vs. C-TTL30 en §2.3bis. Es la primera confirmación independiente, con una métrica distinta, de que el costo de `orjson.dumps` + `SET` en cada miss es real y de esa magnitud.

**Importante — qué NO mide esta métrica, ni siquiera con la corrección:** el cronómetro `with REQUEST_LATENCY.labels(arm=arm).time():` en `main.py:36` termina en el `return data` de la función — **no cubre la validación Pydantic de `response_model=EstadoSiniestro` ni la serialización de `ORJSONResponse`**, que ocurren después, fuera del bloque cronometrado. Eso significa que la diferencia entre esta latencia interna (1.56-2.34ms) y el p95 de k6 (6.19-7.86ms) **no puede llamarse "overhead de transporte"** — incluye la serialización completa de la respuesta, que es justamente lo que el brazo C' (bypass de Pydantic, nunca ejecutado con el script corregido — ver Anexo E, ítem pendiente) estaba diseñado para aislar. No se hace esa resta en esta versión del informe.

> **Amenaza a la validez de esta reconstrucción:** al no existir `run_id` como label, la ventana de cada corrida se estimó por timestamp, no por una fuente que confirme el límite exacto — se verificó que no hay solapamiento entre corridas consecutivas. Además, `_sum/_count` dan la **media**, no un percentil: no permite ninguna afirmación sobre p95/p99 interno, solo sobre el promedio. Los 9 valores de esta tabla ya se exportaron y versionaron en `results/raw/ha08_read_latency_seconds_por_corrida.csv` (delta de `_sum`/`_count` por corrida vía `increase()`), antes del vencimiento de la retención de 7 días de Prometheus (~16-09).

### 2.1quinquies Validación del dictamen bajo condiciones más duras (V1/V3/V6, auditoría externa)

**Hallazgo crítico de la auditoría externa:** `Guia_Tecnica_HA-08.md` §14 clasifica explícitamente un "p95 del brazo A sospechosamente bajo (< 20ms)" como **síntoma a corregir**, con causa probable "el dataset cabe en memoria" y acción prescrita "verificar el tamaño de la base y reducir `shared_buffers`". A midió 7.86ms — cae en esa zona de alarma, y esa acción nunca se había ejecutado. Adicionalmente, el 80% del tráfico (el *hot set*, IDs 1-10.000) corresponde a las primeras filas insertadas por `generate_series` en `db/init/03_seed.sql` — físicamente contiguas en el heap y los índices, la disposición más favorable posible para los `JOIN` de A.

**Se ejecutó una corrida de validación** (`scripts/run_validacion_v1_v3.sh`) reproduciendo ambas condiciones simultáneamente:
- `PG_SHARED_BUFFERS=64MB` (bajado 4× desde 256MB) y `PG_EFFECTIVE_CACHE_SIZE=128MB`, aplicados recreando el contenedor de Postgres (`--force-recreate`; un simple `restart` no aplica el cambio porque el parámetro se fija en `command:` al crear el contenedor). El volumen `pgdata` persiste — no se regeneró el dataset.
- Hot set disperso: `load/k6/read_estado_hotset_disperso.js` (copia de `read_estado.js` con `pickId()` modificado) selecciona claves calientes como múltiplos de 100 en el rango 1-1.000.000, en vez de 1-10.000 consecutivos — mismo tamaño de hot set (10.000 claves), distribuido por todo el heap.

| | A original (256MB, hot set contiguo) | A validación (64MB, hot set disperso) | Diferencia |
|---|---:|---:|---:|
| p95 | 7.86ms | 8.19ms | +0.33ms (+4.2%) |
| Error | 0% | 0% | — |
| `http_reqs.rate` | ~30.8 req/s | 30.77 req/s | criterio de corrida limpia cumplido |
| CPU Postgres (avg/max) | 4.9-5.5% / 9.3-11.1% | 8.6% / 30.2% | ~3× más I/O real forzado |

**El cambio de configuración sí tuvo efecto real** (CPU de Postgres casi se triplicó en el máximo, confirmando que se forzó I/O real en vez de resolver todo desde caché de página), **pero el impacto en el p95 de A es marginal: +4.2%, de 19.1× a 18.3× de margen sobre el umbral de 150ms.** El dictamen de §2.2 **se sostiene** bajo las condiciones que la propia guía técnica del equipo señalaba como necesarias para descartar el síntoma de "dataset cabe en memoria" — A no se acerca al umbral ni con `shared_buffers` reducido 4× ni con el hot set físicamente disperso.

Evidencia: `results/raw/{summary,stats,lag,cache}_A_v1v3_r1.{json,csv,txt}`, `results/raw/log_validacion_v1v3.txt`. Postgres se restauró a `shared_buffers=256MB` (configuración de `.env`) al terminar la corrida.

> Esta validación cubre V1, V3 y V6 con una sola corrida (los tres hallazgos apuntaban al mismo mecanismo: el montaje abarata artificialmente el costo de A). No se ejecutaron V4/B2 (staleness real vía `version`), B3/B5 (thresholds por sub-métrica en el protocolo formal) ni B1 (C') — se evaluó que solo V1/V3/V6 tenían poder de invalidar el dictamen central; los demás son refinamiento de precisión sobre conclusiones ya establecidas, documentados como pendientes en §5/Anexo E.
...
1. **El hit-rate medido con TTL=300 (32-33% en las 3 corridas limpias — `r1b`/`r2`/`r3`) queda por debajo de lo que predice el estado estacionario, porque la corrida nunca llega a ese estado.** Con tráfico uniforme sobre el *hot set* de 10.000 claves, el hit-rate esperado en estado estacionario para una clave con tasa de llegada λ=0.8·rps/10.000 (el factor 0.8 es la fracción de tráfico dirigida al *hot set*) es `λ·TTL / (1 + λ·TTL)`; ponderando por el número de solicitudes de cada escalón (10/25/50/80 rps × 180s cada uno) y por el 20% de tráfico frío (que prácticamente nunca acierta), el hit-rate global teórico para TTL=300 en estado estacionario es **~44%**. El valor medido (32-33%) es sustancialmente menor porque cada corrida arranca con `FLUSHALL` (caché vacío) y cada escalón dura solo 180s — mucho menos que el TTL de 300s — así que el caché nunca termina de llenarse dentro de la ventana de medición: es un régimen transitorio, no estacionario. (Para TTL=30 del protocolo formal, la misma fórmula da ~9.6% en estado estacionario, consistente con el 7.6% medido — ahí sí se alcanza el estado estacionario porque el TTL es corto frente a la duración del escalón.)
   > **Corrección tras auditoría externa (E1):** el rango citado aquí originalmente era 24-33%, que incluía la corrida `ttl300_r1` (contaminada, `http_reqs.rate`=16.5 req/s, ~50% de la carga diseñada) descartada tras la repetición documentada abajo. Con las 3 corridas limpias el rango real es 32-33%, más cerca del 44% teórico de lo que sugería el rango anterior — el argumento del régimen transitorio se sostiene igual de bien, la brecha entre lo medido y lo teórico es más pequeña de lo que el rango original hacía parecer, no más grande.
2. **A pesar del hit-rate más alto (32-33% vs. 0%), TTL=300 es *más lento* que TTL=0, no más rápido** (6.50ms vs. 6.27ms). El costo no está en el `GET` a Redis que falla — comparando series (**nota: B, C-TTL0 y C-TTL30 corrieron en corridas distintas, no simultáneas; esta es una comparación indicativa, no un experimento controlado con la misma corrida**): B promedia 6.19ms, C-TTL0 (que hace el `GET` fallido pero nunca ejecuta el `SET`, por el guard de `settings.cache_ttl_seconds > 0`) promedia 6.27ms — una diferencia de 0.08ms, prácticamente cero. C con TTL=30 del protocolo formal (que sí ejecuta `orjson.dumps` + `SET` en cada miss) promedia 6.72ms — 0.45ms más que C-TTL0. **El costo real está en la serialización y escritura a Redis tras cada miss, no en el `GET` que falla.** Esto sugiere una mejora concreta no implementada en este experimento: escribir en caché de forma asíncrona (fire-and-forget), sin bloquear la respuesta al cliente con el `SET`.
...
El contenedor `k6-carga` consume consistentemente menos CPU que `ha08-api` en los tres brazos — descarta la hipótesis de que k6 se sature antes que la API a esta tasa. El servicio `api` tiene `cpus: "2.0"` en `docker-compose.yml` (no 1 core); el máximo observado (46.5%, relativo a 1 core en `docker stats`) equivale a ~23% del límite real de 2 cores — lejos de saturarlo.
...
**Interpretación:** el punto de quiebre real está por **encima de un pico de 600 req/s** en este montaje (dataset de 1M filas con índices efectivos, CPU de la API lejos de saturar su límite real de 2 cores). No se intentó llegar más alto por el costo de tiempo de correr escalones aún mayores; una alternativa metodológicamente más económica para localizar el quiebre sería reducir deliberadamente el CPU asignado a la API (p. ej. a 0.5 cores) o el tamaño del pool de conexiones (a 2-3), para forzar contención observable a tasas de carga más bajas y comparar cómo se degrada cada brazo — no se ejecutó en esta sesión.
...
| Tasa de error < 1% | ✅ 0% en las 20 corridas ejecutadas (incluye `ttl0_r1`/`ttl300_r1`, descartadas del análisis por contención pero limpias en tasa de error) |
...
- [x] Repetir las variantes de sensibilidad TTL con n=3 cada una — completadas sin errores (ver §2.3bis). Con las 6 corridas limpias (`r1b`/`r2`/`r3` de cada valor): TTL=0 avg=6.27ms, TTL=300 avg=6.50ms — TTL=300 es *más lento* a pesar de mayor hit-rate (32-33% vs. 0%), mismo patrón de §2.1. `ttl0_r1` y `ttl300_r1` originales también mostraron contención (no solo `r2`/`r3` de la primera ronda) y se repitieron como `r1b` (documentado en la nota de proceso de §2.3bis).
...
- [x] **(B4, auditoría externa)** Extraer y publicar la latencia interna de `ha08_read_latency_seconds` por brazo — corregido en una segunda ronda: el primer intento usaba `histogram_quantile()` sobre buckets con resolución mínima de 5ms (inválido, casi toda la masa cae en el primer bucket). Corregido a `increase(_sum)/increase(_count)` (latencia media exacta, no percentil) — ver §2.1quater. A promedia 2.34ms, B 1.56ms, C 1.99ms; C−B=0.42ms coincide con el 0.45ms de costo de escritura ya establecido en §2.3bis (confirmación independiente, no contradicción como en el primer intento). Exportado a `results/raw/ha08_read_latency_seconds_por_corrida.csv` antes del vencimiento de retención de Prometheus.
- [x] **(V1/V3/V6, auditoría externa)** Validar el dictamen de A con `shared_buffers` reducido 4× (64MB) y hot set físicamente disperso — completado, ver §2.1quinquies. **El dictamen se sostiene**: p95 sube de 7.86ms a 8.19ms (+4.2%), margen baja de 19.1× a 18.3× sobre el umbral. CPU de Postgres casi se triplicó (confirma que el cambio forzó I/O real), pero A sigue muy lejos de comprometer `EC-LAT-11`.
