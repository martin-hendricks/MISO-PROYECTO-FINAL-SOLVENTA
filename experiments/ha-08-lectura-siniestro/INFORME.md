...
### 2.1ter Latencia interna vs. latencia de transporte (B4, feedback de auditoría externa)

`ha08_read_latency_seconds` (histograma expuesto por la API, label `arm`, sin `run_id`) mide la latencia *dentro* del servicio, desde que empieza a resolver la estrategia de lectura hasta que arma la respuesta — excluye el tiempo de red/HTTP/ASGI entre k6 y la API, que sí incluye el p95 de k6 (§2.1). Como el label `arm` no distingue `run_id` (misma limitación documentada en §4.4 para el panel de Grafana), se reconstruyó por ventana de tiempo, consultando `histogram_quantile(0.95, sum(rate(ha08_read_latency_seconds_bucket{arm="X"}[780s])) by (le))` al final de cada una de las 9 ventanas del protocolo formal (vía la API HTTP de Prometheus, dentro de la retención de 7 días):

| Brazo | p95 interno por corrida (ms) | Promedio | p95 de k6 (cliente) | Overhead de transporte |
|---|---|---:|---:|---:|
| A | 8.36 / 7.44 / 4.97 | 6.92 | 7.86 | 0.94ms (12%) |
| B | 4.80 / 4.78 / 4.78 | 4.79 | 6.19 | 1.40ms (23%) |
| C | 4.84 / 4.85 / 4.83 | 4.84 | 6.72 | 1.88ms (28%) |

**A es el único brazo con variabilidad alta entre corridas (4.97-8.36ms, rango de 3.4ms) — B y C son extremadamente estables (variación <0.1ms).** Es coherente con la arquitectura: A ejecuta `JOIN`s reales sobre el modelo normalizado en Postgres, sensible al estado de caché de página y planes de consulta de cada arranque de contenedor; B y C resuelven por clave primaria sobre una fila plana, un acceso mucho más uniforme independiente del historial de la corrida.

**Esto es más fino que el p95 de k6 y matiza el hallazgo de §2.1:** medida *dentro* del servicio, A es ~44% más lenta que B (6.92 vs. 4.79ms) — más marcado que el 27% observado con k6 — mientras que B y C son prácticamente indistinguibles internamente (diferencia de 0.05ms, dentro del ruido). El overhead de transporte (12-28%, creciente de A a C) diluye parte de esa diferencia en lo que finalmente ve el cliente. Dicho de otro modo: **la ventaja de B sobre A es real y algo mayor de lo que sugiere el p95 de k6; la desventaja de C frente a B está casi enteramente en el costo de escritura a Redis y el transporte asociado, no en la resolución interna de la estrategia de lectura misma.**

> **Amenaza a la validez de esta reconstrucción:** al no existir `run_id` como label, la ventana de cada corrida se estimó por timestamp (inicio del log + 25s de estabilización + 13 min de duración de k6), no por una fuente que confirme el límite exacto. Se verificó que las ventanas no se solapan entre corridas consecutivas y que el conteo de observaciones (`_count`) es consistente con el volumen esperado de cada corrida, pero no se descarta que algún borde de ventana capture unos segundos de la corrida anterior o siguiente — es la misma limitación que ya afecta al panel "p95 interno por brazo" de Grafana (§4.4).
...
- [x] **(B4, auditoría externa)** Extraer y publicar el p95 de `ha08_read_latency_seconds` (latencia interna del servicio, sin transporte HTTP) por brazo — completado, ver §2.1ter. Hallazgo: A tiene alta variabilidad interna entre corridas (4.97-8.36ms) mientras B/C son extremadamente estables (<0.1ms de variación); medido dentro del servicio A es ~44% más lenta que B (más marcado que el 27% visto con k6), y B/C son prácticamente indistinguibles internamente — la diferencia C vs. B observada por el cliente está casi enteramente en el costo de escritura a Redis + transporte, no en la resolución de la estrategia de lectura misma.
