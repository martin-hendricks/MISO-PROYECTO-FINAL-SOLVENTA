...
>
> **`ttl0_r1` y `ttl300_r1` también muestran contención, no detectada en su momento y no repetida.** El criterio usado para identificar corridas afectadas fue el outlier extremo en `max` (~924s), que r1 de ambos TTL no presenta — pero comparando la tasa de llegada efectiva (`http_reqs.rate` de cada `summary_C_<run>.json`), `ttl0_r1` corrió a 14.1 req/s y `ttl300_r1` a 16.5 req/s, contra 30.8 req/s en las 4 corridas restantes: el escenario de k6 tardó aproximadamente el doble en tiempo de reloj para completar los mismos 24.000 requests, la misma firma de desfase reloj/escenario documentada arriba, solo que sin el outlier extremo asociado. **No se repitieron estas dos corridas en esta versión del informe** (se decidió documentar en vez de re-ejecutar, dejando la repetición para una v2 posterior) — el impacto verificado es menor: excluir `ttl0_r1` del promedio de TTL=0 cambia 6.21→6.30ms, y excluir `ttl300_r1` del promedio de TTL=300 cambia 6.47→6.51ms (diferencias de 0.04-0.09ms), frente a los 0.21-0.26ms de diferencia entre TTL=0 y TTL=300 que sostienen la conclusión de este apartado — el dictamen no cambia con o sin estas dos corridas.
...
A (que ejecuta el `JOIN` completo sobre el modelo normalizado en cada request) impone **~2.5× la carga de CPU sobre Postgres** frente a B/C (que solo leen la proyección materializada), y su máximo (67.7%) se acerca de forma mucho más visible al límite de 2 cores del contenedor que el de B/C (13.4-13.6%). Junto con el p99 de A en r600 (60.16ms, contra 2.54ms de B) de la tabla anterior, esta es la primera evidencia cuantitativa de que **B sí se justifica frente a A — no por latencia (§2.1/§2.1bis), sino por aislamiento de carga sobre el almacén transaccional y margen de crecimiento**: a mayor escala o con más tráfico de escritura concurrente, el camino de A competiría directamente con las transacciones de negocio por los mismos recursos de Postgres, mientras que B (y C) no. Es el "camino 2" que ya se planteaba como opción en §2.2, ahora con datos que lo sostienen.
...
- [ ] **v2 pendiente:** repetir `ttl0_r1` y `ttl300_r1` — muestran la misma firma de contención que `r2`/`r3` (throughput real de 14.1 y 16.5 req/s contra 30.8 req/s normal), detectada tarde y documentada en vez de repetida (ver nota de proceso en §2.3bis). El impacto verificado en el p95 promedio es menor (0.04-0.09ms) y no cambia el dictamen, pero deja la serie de TTL sin el mismo estándar de limpieza que el resto del informe.
